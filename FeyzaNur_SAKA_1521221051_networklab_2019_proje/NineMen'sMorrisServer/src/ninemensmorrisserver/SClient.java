...
public class SClient {
    ...
    //rakip client
    SClient rival;//rakip c1 in rakibi c2, c2 nin c1
    //eşleşme durumu
    ...
    public void Send(Message message) {
        try {
            this.sOutput.writeObject(message);//sclientın outputuna write object
        } catch (IOException ex) {
            Logger.getLogger(SClient.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        public void run() {
            ...
            while (TheClient.soket.isConnected()) {
                try {
                    ...
                    Message received = (Message) (TheClient.sInput.readObject());//isim al 
                    //mesaj gelirse bu satıra geçer
                    //mesaj tipine göre işlemlere ayır
                    switch (received.type) {
                        case Name:
                            TheClient.name = received.content.toString();//conten object tipi
                            // isim verisini gönderdikten sonra eşleştirme işlemine başla
                            TheClient.pairThread.start();//eşleşme için bir thread başlattım yani eş arıyo bir eş gelince thread biter
                            break;
                        case Disconnect:
                            break;
                        case Text:
                        ...
                        case Bitis:
                            break;
//eşleşme olduğunda ben buna send dicem
                    }
    ...
    }//litena kadar algılama işlemi

    //eşleştirme threadi
    //her clientin ayrı bir eşleştirme thredi var
    class PairingThread extends Thread {//eşleştirmek için
        ...
        public void run() {
            ...
            while (TheClient.soket.isConnected() && TheClient.paired == false) {
                try {
                    ...
                    Server.pairTwo.acquire(1);//sadece bir tane thread girebilir. kitliyo

                    //client eğer eşleşmemişse gir
                    if (!TheClient.paired) {//eşi yoksa 
                        SClient crival = null;//rakip
                        //eşleşme sağlanana kadar dön
                        while (crival == null && TheClient.soket.isConnected()) {

                            //liste içerisinde eş arıyor
                            for (SClient clnt : Server.Clients) {//server client ın litesinde dönmeye başlıyo
                                if (TheClient != clnt && clnt.rival == null) {
                                    //eşleşme sağlandı ve gerekli işaretlemeler yapıldı
                                    //gelen thread null ise bu threadı çalıştıran clientı eşleştir
                                    crival = clnt;
                                    crival.paired = true;
                        ...
                        msg1.content = TheClient.name;//clienta
                        Server.Send(TheClient.rival, msg1);

                        Message msg2 = new Message(Message.Message_Type.RivalConnected);
                        msg2.content = TheClient.rival.name;//clientın rivalina gönderiyorum
